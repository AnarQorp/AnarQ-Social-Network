...
export class ThresholdSignatureService extends EventEmitter {
    validatorSetsCache = new Map();
    signatureRequestsCache = new Map();
    criticalOperationsCache = new Map();
    validatorKeysCache = new Map();
     ...
     */
    async initializeValidatorSet(daoSubnet, validators, // Validator identity IDs
    scheme = 'BLS', threshold) {
        try {
            ...
            const keyPairs = await this.generateValidatorKeys(validators, scheme, calculatedThreshold);
            // Create threshold signature scheme
            const thresholdScheme = {
                scheme,
            ...
            const validatorInfos = validators.map((validatorId, index) => ({
                validatorId,
     ...
     */
    async requestThresholdSignature(daoSubnet, message, purpose, metadata, expirationMinutes = 60) {
        try {
     ...
     */
    async submitSignatureShare(signatureId, validatorId, signature, metadata) {
        try {
            ...
            const isValid = await this.verifySignatureShare(signature, signatureRequest.messageHash, validator.publicKey, signatureRequest.scheme.scheme);
            if (!isValid) {
                // Record slashing event for invalid signature
                await this.recordSlashingEvent(validatorId, 'malicious_behavior', 'minor', 'Invalid signature share submitted', 'system');
     ...
     */
    async createCriticalOperation(type, daoSubnet, description, data, initiatedBy, deadlineMinutes = 120) {
        try {
            const validatorSet = this.validatorSetsCache.get(daoSubnet);
            if (!validatorSet) {
                throw new Error('Validator set not found for DAO subnet');
            ...
            const signatureId = await this.requestThresholdSignature(daoSubnet, JSON.stringify({ operationId, type, data }), 'step_commit', {
                initiatedBy,
                criticalOperation: true,
                stepId: operationId
            }, deadlineMinutes);
            criticalOperation.signatures.push(this.signatureRequestsCache.get(signatureId));
     ...
     */
    async getValidatorSet(daoSubnet) {
        try {
     ...
     */
    async getCriticalOperation(operationId) {
        try {
     ...
     */
    async rotateValidatorSet(daoSubnet, newValidators, rotatedBy) {
        try {
            ...
            const newSet = await this.initializeValidatorSet(daoSubnet, newValidators, currentSet.scheme.scheme, currentSet.scheme.threshold);
    ...
    async generateValidatorKeys(validators, scheme, threshold) {
        try {
            const keyPairs = [];
    ...
    async generateAggregatePublicKey(keyPairs, scheme) {
        try {
    ...
    async verifySignatureShare(signature, messageHash, publicKey, scheme) {
        try {
    ...
    async aggregateSignature(signatureRequest) {
        try {
    ...
    async recordSlashingEvent(validatorId, reason, severity, evidence, reportedBy) {
        try {
    ...
    calculateSlashingPenalty(severity) {
        switch (severity) {
    ...
    async updateValidatorReputations() {
        try {
...
//# sourceMappingURL=ThresholdSignatureService.js.map