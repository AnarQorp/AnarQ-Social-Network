...
export class CRDTStateManager extends EventEmitter {
     ...
     */
    async applyOperation(executionId, operation) {
        try {
     ...
     */
    async mergeConcurrentUpdates(updates) {
        try {
     ...
     */
    async getCurrentState(executionId) {
        try {
     ...
     */
    async createStateDelta(executionId, sinceVectorClock) {
        try {
            const crdtState = this.crdtStates.get(executionId);
            if (!crdtState) {
                return null;
            ...
            if (sinceVectorClock) {
                operations = operations.filter(op => this.isOperationNewer(op.vectorClock, sinceVectorClock));
            }
            return {
                executionId,
                nodeId: this.nodeId,
     ...
     */
    async compactState(executionId) {
        try {
            const crdtState = this.crdtStates.get(executionId);
            if (!crdtState) {
                return null;
     ...
     */
    getConflictTelemetry() {
        return [...this.conflictTelemetry].sort((a, b) => new Date(b.resolvedAt).getTime() - new Date(a.resolvedAt).getTime());
    }
    /**
     * Get operation log for execution
     */
    getOperationLog(executionId) {
        return this.operationLog.get(executionId) || [];
    }
    /**
     * Get compaction history
     */
    getCompactionHistory(executionId) {
        return this.compactionHistory.get(executionId) || [];
    }
    // Private helper methods
    async mergeRemoteOperations(update) {
        try {
            ...
            const newOperations = operations.filter(op => !crdtState.operations.some(localOp => localOp.operationId === op.operationId));
            // Add new operations
    ...
    async detectConflicts(executionId, newOperation) {
        try {
            const conflicts = [];
            const crdtState = this.crdtStates.get(executionId);
            if (!crdtState) {
    ...
    async resolveConflicts(executionId, conflicts) {
        try {
    ...
    async resolveConflict(executionId, conflict) {
        ...
        let resolutionStrategy = 'last-write-wins';
        let resolvedValue;
    ...
    async applyOperationToState(state, operation) {
    ...
    sortOperationsByCausalOrder(operations) {
    ...
    isCausallyOrdered(clock1, clock2) {
    ...
    isOperationNewer(opClock, sinceClock) {
    ...
    async loadOrCreateBaseState(executionId) {
        try {
            return await ipfsStateStorage.loadState(executionId);
        }
        catch (error) {
    ...
    async recomputeBaseState(crdtState) {
...
//# sourceMappingURL=CRDTStateManager.js.map