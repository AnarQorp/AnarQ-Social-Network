...
export class AutoScalingEngine extends EventEmitter {
     ...
     */
    async triggerScalingEvaluation() {
    ...
    getScalingStatus() {
        const triggers = Array.from(this.scalingTriggers.values()).map(trigger => {
            ...
            let status = trigger.enabled ? 'active' : 'disabled';
    ...
    setupDefaultPolicies() {
        ...
        const defaultRedirectionPolicies = [
            {
        ...
        defaultRedirectionPolicies.forEach(policy => this.redirectionPolicies.set(policy.id, policy));
        // Default flow pausing policies
        const defaultPausingPolicies = [
            {
        ...
        defaultPausingPolicies.forEach(policy => this.pausingPolicies.set(policy.id, policy));
    }
    ...
    async executeScalingAction(trigger, metricValue) {
        const currentNodeCount = this.nodePool.size;
        let targetNodeCount;
        if (trigger.action === 'scale_up') {
            targetNodeCount = Math.min(Math.ceil(currentNodeCount * trigger.scalingFactor), trigger.maxNodes);
        }
        else {
            targetNodeCount = Math.max(Math.floor(currentNodeCount * trigger.scalingFactor), trigger.minNodes);
        }
    ...
    async evaluateRedirectionPolicies() {
    ...
    async executeLoadRedirection(policy) {
    ...
    async executeFlowPausing(policy) {
    ...
    evaluateRedirectionCondition(condition) {
    ...
    async performScaling(action, currentNodes, targetNodes, trigger) {
    ...
    async performLoadRedirection(policy) {
    ...
    async performFlowPausing(flowIds, policy) {
...
//# sourceMappingURL=AutoScalingEngine.js.map