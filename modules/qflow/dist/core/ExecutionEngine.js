...
import { ErrorType } from '../models/FlowDefinition.js';
...
export class ExecutionEngine {
    ...
    optimizationEnabled;
    constructor(optimizationEnabled = true) {
     ...
     */
    async startExecution(flowId, context) {
        ...
        if (daoContext.daoSubnet) {
            ...
            const billingCheck = await resourceBillingService.canAllocateResources(daoContext.daoSubnet, {
            ...
            // Allocate resources
            const resourceAllocation = await daoSubnetService.allocateResources(daoContext.daoSubnet, daoContext.resourceAllocation);
     ...
     */
    async executeStep(executionId, step, context) {
     ...
     */
    async executeEventTrigger(step) {
     ...
     */
    cleanupExecutions(maxAge = 24 * 60 * 60 * 1000) {
        const now = Date.now();
        const toDelete = [];
     ...
     */
    async releaseDAOResources(execution) {
        try {
            const daoContext = execution.context;
            if (daoContext.daoSubnet && daoContext.resourceAllocation) {
                ...
                await resourceBillingService.trackExecutionUsage(daoContext.daoSubnet, execution.executionId, resourceUsage);
                // Release DAO subnet resources
                await daoSubnetService.releaseResources(daoContext.daoSubnet, daoContext.resourceAllocation);
    ...
    async optimizeFlow(flow) {
        ...
        try {
            ...
            for (const step of optimizedFlow.steps) {
                if (step.lazyLoad) {
                    this.lazyLoadingManager.registerComponent(`step_${step.id}`, async () => {
     ...
     */
    async executeStepsOptimized(steps, context, executionId) {
        if (!this.optimizationEnabled) {
