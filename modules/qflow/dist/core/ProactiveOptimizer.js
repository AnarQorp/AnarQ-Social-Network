...
export class ProactiveOptimizer extends EventEmitter {
    ...
    getOptimizationStatus() {
        const rules = Array.from(this.optimizationRules.values()).map(rule => {
            let status = rule.enabled ? 'active' : 'disabled';
     ...
     */
    async executeOptimization(ruleId, force = false) {
    ...
    evaluateOptimizationTrigger(trigger) {
        try {
            ...
            const connectionWaitTime = Math.max(...Array.from(this.connectionPools.values()).map(pool => pool.averageWaitTime));
    ...
    async performOptimization(rule) {
    ...
    async optimizeValidationParallelism(rule) {
    ...
    async optimizeConnectionPool(rule) {
    ...
    async optimizeResourceAllocation(rule) {
    ...
    async optimizeBatchValidation(rule) {
    ...
    async optimizeResultCaching(rule) {
    ...
    calculateRecentEffectiveness() {
        const recentOptimizations = this.optimizationHistory.filter(opt => Date.now() - opt.timestamp < 3600000 && opt.effectiveness > 0);
        if (recentOptimizations.length === 0) {
            return 0;
        }
        const totalEffectiveness = recentOptimizations.reduce((sum, opt) => sum + opt.effectiveness, 0);
        return totalEffectiveness / recentOptimizations.length;
    }
}
export default ProactiveOptimizer;
//# sourceMappingURL=ProactiveOptimizer.js.map