...
export class LazyLoadingManager extends EventEmitter {
     ...
     */
    registerComponent(id, loader, metadata) {
        const fullMetadata = {
            id,
     ...
     */
    async preloadComponents(context) {
     ...
     */
    async loadDependencies(id) {
        const metadata = this.metadata.get(id);
        if (!metadata || metadata.dependencies.length === 0) {
            return;
        }
        const depPromises = metadata.dependencies.map(depId => this.loadComponent(depId));
     ...
     */
    updateComponentPriority(id, priority) {
        const metadata = this.metadata.get(id);
        if (metadata) {
    ...
    async performLoad(id) {
        ...
        try {
            ...
            const data = await Promise.race([
                loader(),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Load timeout')), this.config.loadTimeout))
            ]);
    ...
    async cacheComponent(id, data, metadata) {
    ...
    async evictLeastUsed(requiredSpace) {
    ...
    shouldPreload(metadata, context) {
...
//# sourceMappingURL=LazyLoadingManager.js.map