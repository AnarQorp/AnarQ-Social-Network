...
export class Libp2pCoordinator extends EventEmitter {
    ...
    subscriptions = new Map(); // topic -> nodeIds
    messageQueue = new Map(); // nodeId -> messages
    pendingDispatches = new Map();
    consensusOperations = new Map();
    networkPartitions = new Map();
    messageHistory = [];
    nodeId;
     ...
     */
    async dispatchExecution(dispatch) {
        try {
     ...
     */
    async startConsensus(type, data, requiredVotes, timeout = 30000) {
        try {
     ...
     */
    async voteOnConsensus(operationId, vote, reason) {
        try {
            const voteObj = {
     ...
     */
    async synchronizeState(executionId, stateData) {
        try {
            const message = {
                messageId: this.generateMessageId(),
    ...
    setupMessageHandlers() {
        this.on('pubsub:message', async (message) => {
            try {
    ...
    async handlePeerDisconnect(event) {
        try {
            const peerId = event.detail.toString();
    ...
    async handleExecutionDispatch(message) {
    ...
    async handleExecutionResult(message) {
    ...
    async handleNodeDiscovery(message) {
        const data = message.data;
        // Update node information
        console.log(`[Libp2pCoordinator] Node discovery: ${data.nodeId}`);
    }
    async handleDispatchTimeout(dispatchId) {
    ...
    async handleConsensusTimeout(operationId) {
    ...
    async handleConsensusReached(operation) {
    ...
    shouldApproveConsensus(operation) {
    ...
    async signVote(operationId, vote) {
...
//# sourceMappingURL=Libp2pCoordinator.js.map