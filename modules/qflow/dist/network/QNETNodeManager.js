...
export class QNETNodeManager extends EventEmitter {
    ...
    async selectNode(criteria) {
        try {
            ...
            if (criteria.daoSubnet) {
                candidateNodes = candidateNodes.filter(node => node.daoSubnets.includes(criteria.daoSubnet));
            }
            ...
            candidateNodes = candidateNodes.filter(node => {
                ...
                const hasDeclaredCapabilities = criteria.requiredCapabilities.every(reqCap => node.capabilities.some(cap => cap.name === reqCap && cap.enabled));
            ...
            candidateNodes = candidateNodes.filter(node => node.performanceScore >= criteria.minPerformanceScore);
            // Filter by latency
            candidateNodes = candidateNodes.filter(node => node.latency <= criteria.maxLatency);
            ...
            if (criteria.geographicPreference && criteria.geographicPreference.length > 0) {
                candidateNodes = candidateNodes.filter(node => criteria.geographicPreference.includes(node.metadata.region || 'unknown'));
            }
            // Exclude specific nodes
            if (criteria.excludeNodes && criteria.excludeNodes.length > 0) {
                candidateNodes = candidateNodes.filter(node => !criteria.excludeNodes.includes(node.nodeId));
            }
            ...
            switch (criteria.loadBalancing || 'performance-based') {
                case 'round-robin':
                    ...
                    break;
                case 'least-loaded':
                    selectedNode = candidateNodes.reduce((best, current) => current.currentLoad < best.currentLoad ? current : best);
                    break;
    ...
    startHealthChecking() {
        ...
        this.healthCheckInterval = setInterval(async () => {
            try {
                ...
                const healthChecks = await Promise.allSettled(nodes.map(nodeId => this.checkNodeHealth(nodeId)));
                let healthyCount = 0;
    ...
    async createNodeFromPeerInfo(peer, nodeInfo) {
...
//# sourceMappingURL=QNETNodeManager.js.map