...
export class WebhookService extends EventEmitter {
     ...
     */
    async processWebhookEvent(endpoint, headers, body, sourceIp) {
        try {
            ...
            if (config.secret || config.qlockVerification?.enabled) {
                const signatureValid = await this.validateSignature(body, config.secret || '', headers[config.signatureHeader || 'x-signature'] || '', config.signatureAlgorithm || 'sha256', config);
                if (!signatureValid) {
                    return {
                        valid: false,
     ...
     */
    async updateWebhookConfig(webhookId, updates, updatedBy) {
        try {
     ...
     */
    async deleteWebhookConfig(webhookId, deletedBy) {
        try {
            const config = this.webhookConfigs.get(webhookId);
            if (!config) {
                return false;
    ...
    findWebhookConfig(endpoint) {
        return Array.from(this.webhookConfigs.values())
            .find(config => config.endpoint === endpoint);
    }
    checkRateLimit(endpoint, limitPerMinute) {
        const now = Date.now();
    ...
    parseWebhookEvent(endpoint, headers, body) {
    ...
    async validateSignature(body, secret, signature, algorithm, config) {
        try {
            if (!signature) {
                return false;
            }
     ...
     */
    async validateQlockSignature(body, signature, config) {
        try {
     ...
     */
    async validateQonsentScopes(event, config, sourceIp) {
        try {
     ...
     */
    async assessQerberosRisk(event, config, sourceIp) {
        try {
     ...
     */
    performBasicRiskAssessment(event, sourceIp) {
        let riskScore = 0;
        const riskFactors = [];
        // Check payload size
     ...
     */
    getRecentEventsFromSource(source) {
    ...
    async validateThroughPipeline(event, config) {
        try {
    ...
    createExecutionContext(event, config) {
        return {
