...
export class ResourceBillingService extends EventEmitter {
    ...
    limitsCache = new Map();
    alertsCache = new Map();
    billingPeriodsCache = new Map();
    isolationConfigCache = new Map();
     ...
     */
    async initializeTenant(tenantId, billingTier = 'free') {
        try {
     ...
     */
    async trackExecutionUsage(tenantId, executionId, resourceUsage) {
        try {
            const currentPeriod = this.getCurrentPeriod();
     ...
     */
    async trackAPIUsage(tenantId, endpoint, responseTime, statusCode) {
        try {
            const currentPeriod = this.getCurrentPeriod();
            const usageKey = `${tenantId}:${currentPeriod}`;
            let usage = this.usageCache.get(usageKey);
     ...
     */
    async canAllocateResources(tenantId, requestedResources) {
        try {
     ...
     * Get tenant alerts
     */
    async getTenantAlerts(tenantId, unacknowledgedOnly = false) {
        try {
     ...
     */
    async updateBillingTier(tenantId, newTier, updatedBy) {
        try {
     ...
     */
    async generateBillingReport(tenantId, period) {
        try {
    ...
    getDefaultIsolationConfig(tenantId, tier) {
        const baseConfig = {
            tenantId,
            dataResidency: {
                region: 'us-east-1',
    ...
    createEmptyUsage(tenantId, period) {
        const now = new Date().toISOString();
        const emptyMetric = (unit) => ({
            current: 0,
            peak: 0,
            average: 0,
            total: 0,
    ...
    async checkLimitsAndAlert(tenantId, usage) {
        try {
            const alerts = [];
    ...
    createAlert(tenantId, resource, currentValue, threshold) {
        const severity = currentValue >= 95 ? 'critical' :
            currentValue >= 90 ? 'high' :
                currentValue >= 85 ? 'medium' : 'low';
        const type = currentValue >= 100 ? 'limit_exceeded' : 'warning';
        return {
    ...
    async calculateProjectedCost(tenantId, requestedResources) {
        try {
            const limits = this.limitsCache.get(tenantId);
            if (!limits) {
                return 0;
...
//# sourceMappingURL=ResourceBillingService.js.map