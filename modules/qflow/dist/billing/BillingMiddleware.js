 ...
 */
export function billingTracker(options = {}) {
    return async (req, res, next) => {
        try {
            const { trackUsage = true, enforceQuotas = true, exemptEndpoints = ['/health', '/metrics', '/docs'] } = options;
            ...
            if (trackUsage) {
                res.on('finish', async () => {
                    if (req.billing && !req.billing.tracked) {
                        ...
                        try {
                            await resourceBillingService.trackAPIUsage(req.billing.tenantId, `${req.method} ${req.path}`, responseTime, res.statusCode);
 ...
 */
export function resourceLimitCheck(resourceType) {
    return async (req, res, next) => {
        try {
            const tenantId = getTenantId(req);
            if (!tenantId) {
            ...
            const quotaCheck = await resourceBillingService.canAllocateResources(tenantId, estimatedResources);
            if (!quotaCheck.allowed) {
                return res.status(429).json({
                    success: false,
 ...
 */
export async function getTenantBillingSummary(req, res) {
    try {
        const tenantId = getTenantId(req);
        if (!tenantId) {
...
function getTenantId(req) {
    ...
    return (req.headers['x-tenant-id'] ||
        req.headers['x-dao-subnet'] ||
        req.query.tenantId ||
        req.body?.daoSubnet ||
        req.identity?.metadata?.daoSubnet ||
        null);
}
function estimateResourceRequirements(req, resourceType) {
    const requirements = {};
...
//# sourceMappingURL=BillingMiddleware.js.map