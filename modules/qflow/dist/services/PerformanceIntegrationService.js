...
export class PerformanceIntegrationService extends EventEmitter {
     ...
     */
    recordFlowExecution(flowId, metrics) {
        const labels = {
            flowId,
        ...
        // Record core metrics
        this.metricsService.recordRequest('qflow', 'flow_execution', metrics.duration, metrics.success ? 200 : 500, labels);
     ...
     */
    recordStepExecution(stepId, metrics) {
        const labels = {
            stepId,
            nodeId: metrics.nodeId,
     ...
     */
    recordValidationMetrics(operationId, metrics) {
    ...
    getPerformanceStatus() {
        ...
        const criticalFailed = failedGates.filter(g => this.performanceGates.get(g.name)?.metric.includes('error') ||
     ...
     */
    async triggerAdaptiveResponse(trigger, context = {}) {
    ...
    setupDefaultResponses() {
        ...
        defaultResponses.forEach(response => this.adaptiveResponses.set(response.trigger, response));
    }
    setupEventHandlers() {
...
//# sourceMappingURL=PerformanceIntegrationService.js.map