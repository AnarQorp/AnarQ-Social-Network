...
export class RealtimeDashboardService extends EventEmitter {
    ...
    updateInterval = null;
    config;
    constructor(performanceService, adaptiveService, options = {}) {
        super();
     ...
     */
    async sendNotification(alertId, alert, channels) {
        const promises = [];
        if (channels.webhook) {
    ...
    handleSetFilters(clientId, data) {
        const client = this.clients.get(clientId);
        if (!client)
            return;
    ...
    passesClientFilters(client, message) {
    ...
    evaluateAlertCondition(condition, context) {
    ...
    setupNotificationChannels(alertId, channels) {
    ...
    async sendSMSNotification(sms, alert) {
