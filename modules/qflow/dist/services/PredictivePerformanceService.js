...
export class PredictivePerformanceService extends EventEmitter {
    ...
    anomalyDetectors;
    config;
    constructor(correlationService, options = {}) {
        super();
     ...
     */
    async generatePerformanceForecast(targetModule, targetMetric, timeHorizon = 60) {
     ...
     */
    async predictAnomalies(targetModule, timeHorizon = 30) {
     ...
     */
    async generateCapacityForecasts(targetModule, timeHorizon = 60) {
     ...
     */
    async getEcosystemPredictions(timeHorizon = 60) {
    ...
    selectBestModel(targetModule, targetMetric) {
    ...
    async generatePredictions(model, targetModule, targetMetric, timeHorizon) {
        const predictions = [];
    ...
    async generateSinglePrediction(model, targetModule, targetMetric, timestamp, baselineValue) {
    ...
    async predictMetricAnomaly(targetModule, metric, timeHorizon) {
        ...
        // Determine severity
        let severity = 'low';
        if (probabilityOfAnomaly > 0.9)
            severity = 'critical';
        else if (probabilityOfAnomaly > 0.7)
            severity = 'high';
        else if (probabilityOfAnomaly > 0.5)
            severity = 'medium';
    ...
    async generateResourceCapacityForecast(targetModule, resource, timeHorizon) {
        ...
        const recommendedActions = this.generateCapacityRecommendations(targetModule, resource, currentUtilization, predictions, capacityExhaustionTime);
        return {
            module: targetModule,
            resource,
            currentUtilization,
    ...
    async getCurrentMetricValue(targetModule, targetMetric) {
    ...
    async getHistoricalValues(targetModule, metric, count) {
    ...
    async getCurrentResourceUtilization(targetModule, resource) {
        // Mock implementation
        return 0.3 + Math.random() * 0.4;
    }
    async getHistoricalResourceUtilization(targetModule, resource, count) {
        // Mock implementation
        const values = [];
    ...
    estimateTimeToAnomaly(currentValue, mean, stdDev, trend) {
    ...
    async identifyContributingFactors(targetModule, metric) {
    ...
    calculateCapacityExhaustionTime(predictions) {
    ...
    generateCapacityRecommendations(targetModule, resource, currentUtilization, predictions, capacityExhaustionTime) {
    ...
    identifyRiskFactors(correlations, targetModule) {
        const riskFactors = [];
        const strongNegativeCorrelations = correlations.filter(c => c.correlationType === 'negative' && c.strength === 'strong');
        ...
        const criticalDependencies = correlations.filter(c => c.impactDirection === 'b_affects_a' && c.strength !== 'weak');
        if (criticalDependencies.length > 2) {
            riskFactors.push(`High dependency on ${criticalDependencies.length} external modules`);
        }
        return riskFactors;
    }
    generateAssumptions(model, correlations) {
        const assumptions = [
    ...
    async predictModuleHealth(moduleId, timeHorizon) {
        // Mock implementation
        const healthScores = ['healthy', 'warning', 'critical'];
        return {
    ...
    identifyModuleRisks(moduleId, correlations, anomalies) {
        const risks = [];
    ...
    async predictEcosystemHealth(timeHorizon) {
...
//# sourceMappingURL=PredictivePerformanceService.js.map