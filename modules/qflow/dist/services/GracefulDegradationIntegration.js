...
export class GracefulDegradationIntegration extends EventEmitter {
    ...
    escalationHistory;
    config;
    constructor(burnRateService, options = {}) {
        super();
     ...
     */
    async manualEscalate(targetLevel, reason) {
     ...
     */
    async manualDeEscalate(targetLevel, reason) {
        if (targetLevel >= this.degradationLadder.currentLevel) {
...
void {
    ...
    this: .burnRateService.on('burn_rate_calculated', (burnRateMetrics) => {
        const metrics = {
            ...
            resourceUtilization: Math.max(burnRateMetrics.resourceBurnRate.cpu, burnRateMetrics.resourceBurnRate.memory)
        };
...
shouldEscalateToLevel(metrics, any, levelData, DegradationLevel);
boolean;
{
    const triggers = levelData.triggers;
    return (metrics.burnRate >= triggers.burnRate ||
...
async;
escalateToLevel(targetLevel, number, reason, string, automatic, boolean);
Promise < void  > {
...
async;
deEscalateToLevel(targetLevel, number, reason, string, automatic, boolean);
Promise < void  > {
    const: previousLevel = this.degradationLadder.currentLevel,
    const: levelData = this.degradationLadder.levels[targetLevel],
    this: .degradationLadder.currentLevel = targetLevel,
...
async;
executeDegradationActions(levelData, DegradationLevel);
Promise < void  > {
...
getActiveActions(levelData, DegradationLevel);
string[];
{
    const actions = [];
    const qflowActions = levelData.actions.qflow;
    const ecosystemActions = levelData.actions.ecosystem;
    if (qflowActions.pauseFlows.enabled)
        actions.push('pause_flows');
    if (qflowActions.deferSteps.enabled)
        actions.push('defer_steps');
    if (qflowActions.reduceParallelism.enabled)
        actions.push('reduce_parallelism');
    if (qflowActions.disableFeatures.enabled)
        actions.push('disable_features');
    if (ecosystemActions.reduceModuleCalls.enabled)
        actions.push('reduce_module_calls');
    if (ecosystemActions.enableCaching.enabled)
        actions.push('enable_caching');
    if (ecosystemActions.limitConnections.enabled)
        actions.push('limit_connections');
    return actions;
}
/**
 * Get degradation ladder configuration
 */
getDegradationLadder();
DegradationLadder;
{
 ...
 */
updateDegradationLadder(ladder, (Partial));
void {
 ...
 */
async;
resetToNormal(reason, string = 'manual_reset');
Promise < void  > {
    : .degradationLadder.currentLevel > 0
};
{
...
//# sourceMappingURL=GracefulDegradationIntegration.js.map