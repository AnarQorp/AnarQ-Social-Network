...
export class ComprehensiveMetricsService extends EventEmitter {
     ...
     */
    recordLatency(operation, latency, labels = {}) {
     ...
     */
    recordRequest(operation, success, labels = {}) {
     ...
     */
    recordCacheOperation(cacheName, hit, responseTime) {
     ...
     */
    recordValidationPipeline(metrics) {
     ...
     */
    getPercentileMetrics(operation) {
        const histogramKey = `latency_${operation}`;
        if (!this.histograms.has(histogramKey)) {
            return null;
        }
     ...
     */
    getErrorBudgetStatus(operation) {
     ...
     */
    getThroughputMetrics(operation) {
        const counterKey = `requests_${operation}`;
     ...
     * Get metric history
     */
    getMetricHistory(name, timeRange = 3600000) {
        const series = this.metricSeries.get(name);
        if (!series)
            return [];
    ...
    createMetricSeries(name, type, description, unit) {
    ...
    calculatePercentiles(histogramKey) {
    ...
    updateErrorBudget(operation, success) {
    ...
    getRecentErrorRate(operation, timeRange) {
        const errorKey = `errors_${operation}`;
...
//# sourceMappingURL=ComprehensiveMetricsService.js.map