...
class MockQonsentService {
    ...
    async checkPermission(request) {
        ...
        const matchingPermission = userPermissions.find(permission => permission.resource === request.resource &&
    ...
    async renewPermission(permissionId, userId) {
...
export class QonsentValidationLayer {
     ...
     */
    async validatePermissions(data, context) {
        ...
        try {
            ...
            const permissionResults = await Promise.all(permissionRequests.map(request => this.checkPermissionWithCache(request)));
     ...
     */
    async checkPermissionWithCache(request) {
     ...
     */
    extractPermissionRequests(data, context) {
        const requests = [];
     ...
     */
    generatePermissionCacheKey(request) {
     ...
     */
    determineConsentStatus(consentRecord) {
        if (!consentRecord) {
            return 'missing';
        }
...
//# sourceMappingURL=QonsentValidationLayer.js.map