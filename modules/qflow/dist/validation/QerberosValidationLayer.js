...
class MockQerberosService {
    ...
    async validateSecurity(data, context, policyId = 'qflow-security-policy-v1') {
    ...
    async detectAnomalies(data, context) {
        ...
        if (data.steps && Array.isArray(data.steps) && baseline.commonStepTypes) {
            const stepTypes = data.steps.map((step) => step.type).filter(Boolean);
            const unusualTypes = stepTypes.filter((type) => !baseline.commonStepTypes.includes(type));
    ...
    async checkIntegrity(data, context) {
    ...
    async applySecurityRule(rule, data, context) {
        const violations = [];
        switch (rule.type) {
            case 'pattern':
                if (rule.pattern) {
    ...
    async checkBehaviorRule(rule, data, context) {
        const violations = [];
    ...
    async checkIntegrityRule(rule, data, context) {
        const violations = [];
    ...
    async checkAnomalyRule(rule, data, context) {
        const violations = [];
    ...
    generateRecommendations(violations, riskLevel) {
...
export class QerberosValidationLayer {
     ...
     * Get security policy
     */
    async getPolicy(policyId = 'qflow-security-policy-v1') {
        try {
    ...
    getSecurityStats() {
        return {
            policiesLoaded: this.qerberosService.policies.size,
...
//# sourceMappingURL=QerberosValidationLayer.js.map