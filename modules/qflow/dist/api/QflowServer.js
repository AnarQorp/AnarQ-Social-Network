...
import { createAuthMiddleware, requireAuth, requireFlowPermissions, requireFlowOwnership, optionalAuth } from '../auth/AuthMiddleware.js';
...
import { billingTracker, tenantIsolation, billingContext, getTenantBillingSummary } from '../billing/BillingMiddleware.js';
...
export class QflowServer {
    ...
    async handleCreateFlow(req, res) {
        try {
            const { flowData, format = 'auto' } = req.body;
            if (!flowData) {
    ...
    async handleListFlows(req, res) {
        try {
            const { category, visibility, owner, tags, limit = '50', offset = '0' } = req.query;
            ...
            if (tags) {
                ...
                flows = flows.filter(flow => tagList.some(tag => flow.metadata.tags.includes(tag)));
            }
            // Apply pagination
    ...
    async handleDeleteFlow(req, res) {
        try {
            const { id } = req.params;
            const flow = this.flows.get(id);
            if (!flow) {
    ...
    async handleValidateFlow(req, res) {
        try {
            const { flowData, format = 'auto' } = req.body;
            if (!flowData) {
                return this.sendError(res, 'MISSING_FLOW_DATA', 'Flow data is required', 400);
    ...
    async handleStartExecution(req, res) {
        try {
    ...
    async handleListExecutions(req, res) {
        try {
            const { flowId, status, triggeredBy, limit = '50', offset = '0' } = req.query;
    ...
    async handlePauseExecution(req, res) {
        try {
            const { id } = req.params;
    ...
    async handleResumeExecution(req, res) {
        try {
            const { id } = req.params;
    ...
    async handleAbortExecution(req, res) {
        try {
            const { id } = req.params;
    ...
    async handleTransferOwnership(req, res) {
        try {
            ...
            const success = await flowOwnershipService.transferFlowOwnership(flowId, req.identity.id, newOwner, reason, signature);
            if (!success) {
    ...
    async handleGrantAccess(req, res) {
        try {
            const flowId = req.params.id;
            ...
            const success = await flowOwnershipService.grantPermission(flowId, req.identity.id, grantedTo, permission, expiresAt, conditions);
            if (!success) {
    ...
    async handleRevokeAccess(req, res) {
        try {
            const flowId = req.params.id;
            ...
            const success = await flowOwnershipService.revokePermission(flowId, req.identity.id, revokedFrom, permission);
            if (!success) {
    ...
    async handleGetOwnership(req, res) {
        try {
            const flowId = req.params.id;
            if (!req.identity) {
                return this.sendError(res, 'AUTHENTICATION_REQUIRED', 'Identity required', 401);
    ...
    async handleCreateDAOSubnet(req, res) {
        try {
    ...
    async handleListDAOSubnets(req, res) {
        try {
            if (!req.identity) {
                return this.sendError(res, 'AUTHENTICATION_REQUIRED', 'Identity required', 401);
            }
    ...
    async handleGetDAOSubnet(req, res) {
        try {
    ...
    async handleAddValidator(req, res) {
        try {
            const subnetId = req.params.id;
    ...
    async handleCreateProposal(req, res) {
        try {
            const subnetId = req.params.id;
    ...
    async handleVoteOnProposal(req, res) {
        try {
    ...
    async handleGetResourceUsage(req, res) {
        try {
            const subnetId = req.params.id;
            if (!req.identity) {
                return this.sendError(res, 'AUTHENTICATION_REQUIRED', 'Identity required', 401);
    ...
    async handleGetBillingUsage(req, res) {
        try {
            ...
            const usage = await resourceBillingService.getTenantUsage(req.billing.tenantId, period);
            if (!usage) {
    ...
    async handleGetBillingAlerts(req, res) {
        try {
            const { unacknowledged } = req.query;
            if (!req.billing?.tenantId) {
                return this.sendError(res, 'TENANT_ID_REQUIRED', 'Tenant ID is required', 400);
            }
            const alerts = await resourceBillingService.getTenantAlerts(req.billing.tenantId, unacknowledged === 'true');
            this.sendSuccess(res, {
    ...
    async handleUpdateBillingTier(req, res) {
        try {
            const { tier } = req.body;
            if (!req.identity) {
                return this.sendError(res, 'AUTHENTICATION_REQUIRED', 'Identity required', 401);
            ...
            const success = await resourceBillingService.updateBillingTier(req.billing.tenantId, tier, req.identity.id);
            if (!success) {
    ...
    async handleGetBillingReport(req, res) {
        try {
            ...
            const report = await resourceBillingService.generateBillingReport(req.billing.tenantId, period);
            if (!report) {
...
async;
handleGetBillingUsage(req, Request, res, Response);
Promise < void  > {
    try: {
        const: { period } = req.query,
        if(, req) { }, : .billing?.tenantId
...
async;
handleGetBillingAlerts(req, Request, res, Response);
Promise < void  > {
    try: {
        const: { unacknowledged } = req.query,
        if(, req) { }, : .billing?.tenantId
...
async;
handleAcknowledgeAlert(req, Request, res, Response);
Promise < void  > {
    try: {
        const: alertId = req.params.id,
        if(, req) { }, : .identity
...
async;
handleUpdateBillingTier(req, Request, res, Response);
Promise < void  > {
    try: {
        const: { tier } = req.body,
        if(, req) { }, : .identity
...
async;
handleGetBillingReport(req, Request, res, Response);
Promise < void  > {
    try: {
        const: period = req.params.period,
        if(, req) { }, : .billing?.tenantId
...
async;
handleSystemInfo(req, Request, res, Response);
Promise < void  > {
    this: .sendSuccess(res, {
...
async;
handleSystemMetrics(req, Request, res, Response);
Promise < void  > {
...
async;
handleApiDocs(req, Request, res, Response);
Promise < void  > {
    const: docs = {
...
async;
handleDashboardUI(req, Request, res, Response);
Promise < void  > {
    try: {
...
async;
handleDashboardStats(req, Request, res, Response);
Promise < void  > {
    try: {
...
async;
handleDashboardData(req, Request, res, Response);
Promise < void  > {
    try: {
...
async;
handleCreateAlert(req, Request, res, Response);
Promise < void  > {
    try: {
...
async;
handleListAlerts(req, Request, res, Response);
Promise < void  > {
    try: {
...
async;
handleDeleteAlert(req, Request, res, Response);
Promise < void  > {
    try: {
        const: { id } = req.params,
...
async;
handleCacheStats(req, Request, res, Response);
Promise < void  > {
    try: {
...
async;
handleCacheInvalidate(req, Request, res, Response);
Promise < void  > {
    try: {
...
async;
handleCacheClear(req, Request, res, Response);
Promise < void  > {
    try: {
...
async;
handleCachePatterns(req, Request, res, Response);
Promise < void  > {
    try: {
...
sendSuccess(res, Response, data, T, statusCode, number = 200);
void {
    const: response
};
{
...
sendError(res, Response, code, string, message, string, statusCode, number = 400, details ?  : any);
void {
...
async;
canAccessFlow(flow, FlowDefinition, identityId, string);
Promise < boolean > {
...
//# sourceMappingURL=QflowServer.js.map