...
export class DAOCodeTemplateManager extends EventEmitter {
    templates = new Map();
    submissions = new Map();
    whitelistedTemplates = new Set();
    deprecatedTemplates = new Set();
    governanceConfig;
    isRunning = false;
    constructor(config = {}) {
        super();
     ...
     */
    async submitTemplate(templateData, submitter) {
     ...
     */
    async voteOnTemplate(templateId, voter, vote, weight = 1, reason) {
     ...
     */
    getApprovedTemplates(daoSubnet) {
     ...
     */
    async updateTemplateUsage(templateId, success, executionTimeMs) {
        const template = this.templates.get(templateId);
        if (!template) {
            return;
        }
     ...
     * Deprecate template
     */
    async deprecateTemplate(templateId, reason, deprecatedBy) {
        const template = this.templates.get(templateId);
        if (!template) {
    ...
    setupEventHandlers() {
        ...
        this.on('template:security-scan-complete', async (templateId) => {
    ...
    async performSecurityScan(template) {
    ...
    async performStaticAnalysis(template) {
        const issues = [];
        let score = 100;
    ...
    async performDependencyCheck(template) {
        const issues = [];
        let score = 100;
    ...
    async performVulnerabilityScan(template) {
        const issues = [];
        let score = 100;
    ...
    async checkAutoDecision(template) {
    ...
    async checkApprovalThreshold(template) {
    ...
    async performPeriodicSecurityScans() {
    ...
    async signVote(templateId, vote, voter) {
...
//# sourceMappingURL=DAOCodeTemplateManager.js.map