...
export function createAuthMiddleware(options = {}) {
    const { required = true, permissions = [], allowSubIdentities = true, skipForPaths = [] } = options;
    return async (req, res, next) => {
        try {
            ...
            if (permissions.length > 0) {
                ...
                if (!hasPermissions) {
                    return sendAuthError(res, 'INSUFFICIENT_PERMISSIONS', `Required permissions: ${permissions.join(', ')}`, 403, {
...
function sendAuthError(res, code, message, statusCode, details) {
    const response = {
        success: false,
        error: {
            code,
