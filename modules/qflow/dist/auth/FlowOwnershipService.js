...
import { squidIdentityService } from './SquidIdentityService.js';
export class FlowOwnershipService extends EventEmitter {
    ownershipCache = new Map();
    permissionsCache = new Map();
    sharingPoliciesCache = new Map();
    accessRequestsCache = new Map();
     ...
     */
    async registerFlowOwnership(flowId, owner, flowDefinition) {
        try {
     ...
     */
    async transferFlowOwnership(flowId, currentOwner, newOwner, reason, signature) {
        try {
     ...
     */
    async grantPermission(flowId, grantedBy, grantedTo, permission, expiresAt, conditions) {
        try {
            ...
            const filteredPermissions = existingPermissions.filter(p => !(p.grantedTo === grantedTo && p.permission === permission));
            // Add new permission
     ...
     */
    async revokePermission(flowId, revokedBy, revokedFrom, permission) {
        try {
            ...
            const updatedPermissions = existingPermissions.filter(p => !(p.grantedTo === revokedFrom && p.permission === permission));
            this.permissionsCache.set(flowId, updatedPermissions);
     ...
     */
    async hasPermission(identityId, flowId, permission, context) {
        try {
            ...
            const relevantPermissions = permissions.filter(p => p.grantedTo === identityId &&
     ...
     */
    async isFlowOwner(flowId, identityId) {
        try {
            const ownership = await this.getFlowOwnership(flowId);
     ...
     */
    async getFlowPermissions(flowId) {
        try {
     ...
     */
    async requestAccess(flowId, requestedBy, requestedPermissions, reason) {
        try {
     ...
     */
    async reviewAccessRequest(requestId, decision, reviewedBy, reviewNotes) {
        try {
            // Find the request
            let request;
            let flowId;
            ...
            if (decision === 'approved') {
                ...
                for (const permission of request.requestedPermissions) {
                    await this.grantPermission(flowId, reviewedBy, request.requestedBy, permission, expiresAt);
                }
            }
            // Emit review event
     ...
     */
    async updateSharingPolicy(flowId, updatedBy, policy) {
        try {
     ...
     */
    async getOwnedFlows(identityId) {
        try {
            const ownedFlows = [];
     ...
     */
    async getAccessibleFlows(identityId, permission = 'read') {
        try {
            const accessibleFlows = [];
            // Check owned flows
    ...
    async cleanupExpired() {
        try {
            ...
            for (const [flowId, permissions] of this.permissionsCache.entries()) {
                const validPermissions = permissions.filter(p => !p.expiresAt || new Date(p.expiresAt) > now);
            ...
            for (const [flowId, requests] of this.accessRequestsCache.entries()) {
                const validRequests = requests.filter(r => new Date(r.requestedAt).getTime() > (now.getTime() - maxAge));
...
//# sourceMappingURL=FlowOwnershipService.js.map