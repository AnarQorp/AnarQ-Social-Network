...
export class DAOSubnetService extends EventEmitter {
    subnetsCache = new Map();
    policiesCache = new Map();
    proposalsCache = new Map();
    resourceUsageCache = new Map();
     ...
     */
    async registerDAOSubnet(subnetData, createdBy) {
        try {
     ...
     */
    async validateDAOExecution(flowId, executionContext) {
        try {
            const validations = [];
     ...
     */
    async allocateResources(daoSubnet, requestedResources) {
        try {
     ...
     */
    async releaseResources(daoSubnet, releasedResources) {
        try {
            const currentUsage = this.resourceUsageCache.get(daoSubnet);
            if (!currentUsage) {
                return;
     ...
     */
    async addValidator(daoSubnet, validatorData, addedBy) {
        try {
            const subnet = await this.getDAOSubnet(daoSubnet);
            if (!subnet) {
     ...
     */
    async createProposal(daoSubnet, proposalData, proposedBy) {
        try {
            const subnet = await this.getDAOSubnet(daoSubnet);
            if (!subnet) {
                throw new Error('DAO subnet not found');
     ...
     */
    async listAccessibleSubnets(identityId) {
        try {
            const accessibleSubnets = [];
    ...
    createDefaultPolicies(subnetId, createdBy) {
        return [
            {
    ...
    async checkProposalResolution(proposal, subnet) {
        try {
    ...
    async executeProposal(proposal, subnet) {
        try {
...
//# sourceMappingURL=DAOSubnetService.js.map