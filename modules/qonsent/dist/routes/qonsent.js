...
exports.qonsentRoutes = void 0;
const QonsentService_1 = require("../services/QonsentService");
const errors_1 = require("../utils/errors");
const logger_1 = require("../utils/logger");
const qonsentRoutes = async (fastify) => {
    const qonsentService = new QonsentService_1.QonsentService();
    ...
    fastify.post('/check', {
        ...
        try {
            const { resource, identity, action, context } = request.body;
            // Add request context
            ...
            logger_1.logger.error('Permission check failed', { error, body: request.body });
            throw error;
        }
    });
    ...
    fastify.post('/grant', {
        ...
        try {
            const { resource, identity, permissions, expiresAt, conditions } = request.body;
            const grantedBy = request.identity?.squidId;
            if (!grantedBy) {
                throw new errors_1.QonsentError(errors_1.ErrorCodes.INSUFFICIENT_PERMISSIONS, 'Valid identity required to grant permissions');
            }
            ...
            logger_1.logger.error('Permission grant failed', { error, body: request.body });
            throw error;
        }
    });
    ...
    fastify.post('/revoke', {
        ...
        try {
            const { resource, identity, permissions, reason } = request.body;
            const revokedBy = request.identity?.squidId;
            if (!revokedBy) {
                throw new errors_1.QonsentError(errors_1.ErrorCodes.INSUFFICIENT_PERMISSIONS, 'Valid identity required to revoke permissions');
            }
            ...
            logger_1.logger.error('Permission revocation failed', { error, body: request.body });
            throw error;
        }
    });
    ...
    fastify.get('/permissions', {
        ...
        try {
            const { resource, identity, limit, offset } = request.query;
            ...
            logger_1.logger.error('Permission listing failed', { error, query: request.query });
            throw error;
        }
    });
};
exports.qonsentRoutes = qonsentRoutes;
//# sourceMappingURL=qonsent.js.map