...
const UCANPolicyEngine_1 = require("../services/UCANPolicyEngine");
const errors_1 = require("../utils/errors");
const logger_1 = require("../utils/logger");
const policyRoutes = async (fastify) => {
    const policyEngine = new UCANPolicyEngine_1.UCANPolicyEngine();
    // List policies
    fastify.get('/', {
        schema: {
            tags: ['Policies'],
        ...
        try {
            const { resource, identity, scope, limit = 50, offset = 0 } = request.query;
            ...
            logger_1.logger.error('Policy listing failed', { error, query: request.query });
            throw error;
        }
    });
    // Create policy
    fastify.post('/', {
        ...
        try {
            const { name, description, scope, rules, expiresAt } = request.body;
            const createdBy = request.identity?.squidId;
            if (!createdBy) {
                throw new errors_1.QonsentError(errors_1.ErrorCodes.INSUFFICIENT_PERMISSIONS, 'Valid identity required to create policies');
            }
            ...
            logger_1.logger.error('Policy creation failed', { error, body: request.body });
            throw error;
        }
    });
    // Update policy
    fastify.put('/:policyId', {
        ...
        try {
            ...
            if (!updatedBy) {
                throw new errors_1.QonsentError(errors_1.ErrorCodes.INSUFFICIENT_PERMISSIONS, 'Valid identity required to update policies');
            }
            ...
            logger_1.logger.error('Policy update failed', { error, params: request.params, body: request.body });
            throw error;
        }
    });
    // Delete policy
    fastify.delete('/:policyId', {
        ...
        try {
            ...
            if (!deletedBy) {
                throw new errors_1.QonsentError(errors_1.ErrorCodes.INSUFFICIENT_PERMISSIONS, 'Valid identity required to delete policies');
            }
            ...
            logger_1.logger.error('Policy deletion failed', { error, params: request.params });
            throw error;
        }
    });
};
