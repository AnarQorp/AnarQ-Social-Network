...
async function rateLimitMiddleware(fastify) {
    const redis = (0, redis_1.getRedisClient)();
    async function rateLimit(key, options) {
        const now = Date.now();
    ...
    fastify.addHook('preHandler', async (request, reply) => {
        ...
        try {
            ...
            let rateLimitOptions = {
                ...
                window: config_1.config.rateLimitWindow,
                skipOnError: true
            };
            // Higher limits for authenticated users
            if (request.user?.squidId) {
                rateLimitOptions.max = config_1.config.rateLimitMax * 2;
            }
